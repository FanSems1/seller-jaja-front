...
import { Input, Select, DatePicker, Upload, InputNumber, Switch, Spin } from 'antd';
...
function EditPromoJaja() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await apiFetch(PROMO_JAJA_ENDPOINTS.DETAIL(id), { method: 'GET' });
        const data = res.data || res; // tolerate different shapes
        setKodePromo(data.kode_promo || '');
        setJudulPromo(data.judul_promo || '');
        setTipeVoucher(data.tipe_voucher || 'global');
        setKategoriPromo(data.kategori_promo || data.kategori || 'diskon');
        setDateRange([data.mulai ? dayjs(data.mulai) : null, data.berakhir ? dayjs(data.berakhir) : null]);
        // discount mode detection
        if (data.persentase_diskon !== null && data.persentase_diskon !== undefined && String(data.persentase_diskon).trim() !== '') {
          setDiscountMode('persentase');
          setPersentaseDiskon(Number(data.persentase_diskon) || 0);
          setNominalDiskon(0);
        } else {
          setDiscountMode('nominal');
          setNominalDiskon(Number(data.nominal_diskon) || 0);
          setPersentaseDiskon(0);
        }
        setMinBelanja(Number(data.min_belanja) || 0);
        setMaxDiskon(Number(data.max_diskon) || 0);
        setKuota(Number(data.kuota) || 0);
        setBannerPreview(data.banner_url || null);
      } catch (err) {
        Swal.fire({ icon: 'error', title: 'Error', text: err.message || 'Gagal mengambil data', showConfirmButton: false, timer: 2000 });
      } finally {
        setLoading(false);
      }
    };
  ...
  const validate = () => {
    ...
    if (!dateRange || dateRange.length !== 2) return 'Periode wajib diisi';
  ...
  const handleSubmit = async () => {
    const err = validate();
    if (err) { Swal.fire({ icon: 'error', title: 'Gagal', text: err, showConfirmButton: false, timer: 2000 }); return; }
    ...
    try {
      setSaving(true);
      const res = await apiFetch(PROMO_JAJA_ENDPOINTS.UPDATE(id), { method: 'PUT', body: formData });
      const msg = (res && (res.message || res.msg)) || 'Promo berhasil diperbarui';
      Swal.fire({ icon: 'success', title: 'Berhasil', text: msg, showConfirmButton: false, timer: 1500 });
      navigate('/promosi/voucher-jaja');
    } catch (err) {
  ...
  if (loading) return <div className="py-12"><Spin size="large" /></div>;
      ...
      <Card>
        <CardBody>
          <div className="flex items-center justify-between mb-4">
            <div>
              <Typography variant="h5" className="font-bold">Edit Promo Jaja</Typography>
              <Typography className="text-sm text-gray-500">Ubah data promo dan simpan perubahan</Typography>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex justify-end gap-3 mt-4">
              <Button variant="outlined" color="gray" onClick={() => navigate('/promosi/voucher-jaja')}>Batal</Button>
